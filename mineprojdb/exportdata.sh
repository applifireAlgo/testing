...
echo $PATH
OSNAME=`uname -s`
ART_DATA_PATH=/tmp/applifire/db/S3FMJFG5E6ROOCUM3CMKBA/18D01ABF-F632-496A-B379-FC50EDEAB8C0/art/data
AST_DATA_PATH=/tmp/applifire/db/S3FMJFG5E6ROOCUM3CMKBA/18D01ABF-F632-496A-B379-FC50EDEAB8C0/ast/data
DB_NAME=applifire
USER=root
PASSWORD=root
PORT=3306
HOST=localhost
MYSQL=/usr/bin
PROJECT_ID=S3FMJFG5E6ROOCUM3CMKBA
...
$MYSQL/mysql -p$PORT -u$USER -p$PASSWORD $DB_NAME -e "SELECT query_id, jpql_query, query_type, query_json, name, UPPER(name), app_creator_id, project_id, project_version_id, 0, CURRENT_TIMESTAMP, 0, CURRENT_TIMESTAMP, 0, 1, sql_query, user_access FROM aws_query WHERE project_id = '$PROJECT_ID' AND project_version_id = '$PROJECT_VERSION_ID' INTO OUTFILE '$ART_DATA_PATH/art_query.csv' FIELDS TERMINATED BY '#appfire#' LINES TERMINATED BY '\n'";
...
$MYSQL/mysql -p$PORT -u$USER -p$PASSWORD $DB_NAME -e "SELECT severity_id, log_config_id, severity, label, version_id, created_by, created_date, updated_by, updated_date, active_status FROM aws_log_severity_m WHERE project_id = '$PROJECT_ID' AND project_version_id = '$PROJECT_VERSION_ID' INTO OUTFILE '$ART_DATA_PATH/art_log_events_t.csv' FIELDS TERMINATED BY '#appfire#' LINES TERMINATED BY '\n'";
