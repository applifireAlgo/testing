Ext.define('Mineproj.view.fw.frameworkController.FrameworkViewController', {
    ...
    onConsoleClick: function() {
         var window = Ext.create('Mineproj.view.console.ConsoleWindow');
     	  ...
     	  var bindArr = item.bindable.split('.');
   		  ...
   		  for (var index = 0; index < bindArr.length; index++) {
  				...
  				if(index == bindArr.length-1){
  					if(item.xtype == "checkboxsgroups" || item.xtype == "customdatetimefield"){
						level[array_element] = item.getValues();
					} else if(item.xtype == "datefield"){
						level[array_element] = item.getSubmitValue() + " IST";
					} else{
